...
public class JewelObj : MonoBehaviour {
    ...
    IEnumerator _ReGroup(Vector2 pos)
    {
        ...
        Util.MoveTo(this.gameObject, pos, DELAY);

        StartCoroutine(_Destroy());
    }
    IEnumerator _Destroy()
    {
        GribManager.cell.GribCellObject[(int)jewel.JewelPosition.x, (int)jewel.JewelPosition.y].CellTypeProcess();
    ...
    public void getNewPosition()
    {
        ...
        {
            if (GribManager.cell.Map[x, y] != 0 && GribManager.cell.GribCellObject[x, y].cell.CellEffect != 4 && JewelSpawner.spawn.JewelGribScript[x, y] == null)
                newpos = y;
            else if (GribManager.cell.Map[x, y] != 0 && GribManager.cell.GribCellObject[x, y].cell.CellEffect == 4)
            {
                break;
            }
        }
        ...
        if (oldpos != newpos)
            StartCoroutine(Util.IEDrop(this.gameObject, jewel.JewelPosition, GameController.DROP_SPEED));
    }
    public List<JewelObj> GetRow(Vector2 Pos, int type, JewelObj bonus)
    {
        ...
        {
            return Util.ListPlus(tmp1, tmp2, bonus);
        }
    ...
    List<JewelObj> GetLeft(Vector2 Pos, int type)
    {
        ...
        for (int x = (int)Pos.x - 1; x >= 0; x--)
        {
            ...
            if (x != jewel.JewelPosition.x && JewelSpawner.spawn.JewelGribScript[x, (int)Pos.y] != null && JewelSpawner.spawn.JewelGribScript[x, (int)Pos.y].jewel.JewelType == type && GribManager.cell.GribCellObject[x, (int)Pos.y].cell.CellEffect == 0)
    ...
    List<JewelObj> GetTop(Vector2 Pos, int type)
    {
        ...
        {
            if (y != jewel.JewelPosition.y && JewelSpawner.spawn.JewelGribScript[(int)Pos.x, y] != null && JewelSpawner.spawn.JewelGribScript[(int)Pos.x, y].jewel.JewelType == type && GribManager.cell.GribCellObject[(int)Pos.x, y].cell.CellEffect == 0)
    ...
    {
        List<JewelObj> list = Util.ListPlus(GetRow(jewel.JewelPosition, jewel.JewelType, null),
