...
public class GribManager : MonoBehaviour {
    ...
    public CellObject[,] GribCellObject;
...
# if UNITY_EDITOR
...
#endif
        string[] strinresult = mapStringdata.Split(new char[] { '	', '\n' });
        int dem = 0;
        for (int y = 8; y >= 0; y--)
        {
            for (int x = 0; x < 7; x++)
            {
                tmp[x, y] = int.Parse(strinresult[dem]);
                dem++;
            }
        }
        return tmp;
    ...
    void GribCreate(int[,] map)
    {
        ...
        GribCellObject = new CellObject[7, 9];
        for (int x = 0; x < 7; x++)
        { 
          for(int y=0;y<9;y++)
          {
