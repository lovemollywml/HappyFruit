...
public class Supporter : MonoBehaviour {
    ...
    public bool isNoMoreMove()
    {
        ...
        for (int x = 0; x < 7; x++)
        {
            for (int y = 0; y < 9; y++)
            {
                if (JewelSpawner.spawn.JewelGribScript[x, y] != null && GribManager.cell.GribCellObject[x, y].cell.CellEffect == 0)
                {
    ...
    JewelObj MoveChecker(int x, int y, JewelObj obj)
    {
        ...
        foreach (Vector2 item in vtmplist)
        {
            ...
            {
                List<JewelObj> NeiObj1 = Util.ListPlus(obj.GetCollumn(item, obj.jewel.JewelType, null),
    ...
    List<Vector2> getListPos(int x, int y)
    {
        ...
        if (y + 1 < 9 && GribManager.cell.GribCellObject[x, y + 1] != null && GribManager.cell.GribCellObject[x, y + 1].cell.CellEffect == 0)
            vtmplist.Add(new Vector2(x, y + 1));
        if (y - 1 >= 0 && GribManager.cell.GribCellObject[x, y - 1] != null && GribManager.cell.GribCellObject[x, y - 1].cell.CellEffect == 0)
            vtmplist.Add(new Vector2(x, y - 1));
        if (x + 1 < 7 && GribManager.cell.GribCellObject[x + 1, y] != null && GribManager.cell.GribCellObject[x + 1, y].cell.CellEffect == 0)
            vtmplist.Add(new Vector2(x + 1, y));
        if (x - 1 >= 0 && GribManager.cell.GribCellObject[x - 1, y] != null && GribManager.cell.GribCellObject[x - 1, y].cell.CellEffect == 0)
            vtmplist.Add(new Vector2(x - 1, y));
        return vtmplist;
    }
