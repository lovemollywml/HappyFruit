...
public class SpawnController : MonoBehaviour {
    ...
    void Drop()
    {
        for (int y = 0; y < 9; y++)
        {
            for (int x = 0; x < 7; x++)
            {
                if (JewelSpawner.spawn.JewelGribScript[x, y] != null && GribManager.cell.GribCellObject[x, y].cell.CellEffect != 4)
    ...
    void Spawn() 
    {
        ...
        for (int x = 0; x < 7; x++)
        {
            ...
            for (int y = 0; y < 9; y++)
            {
                if (GribManager.cell.GribCellObject[x, y] != null && GribManager.cell.GribCellObject[x, y].cell.CellEffect == 4)
                    s = y + 1;
            }
            for (int y = s; y < 9; y++)
            {
                if (GameController.action.GameState == (int)Timer.GameState.PLAYING)
                    if (GribManager.cell.GribCellObject[x, y] != null && JewelSpawner.spawn.JewelGribScript[x, y] == null)
                    {
                        ...
                        h[x]++;
                        StartCoroutine(Util.IEDrop(tmp, new Vector2(x, y), GameController.DROP_SPEED));
