...
public class GameController : MonoBehaviour {
    ...
    public void RuleChecker(GameObject obj1, GameObject obj2)
    {
        ...
        List<JewelObj> NeiObj1 = Util.ListPlus(Jewel1.GetCollumn(Jewel2.jewel.JewelPosition, Jewel1.jewel.JewelType, null),
                                         Jewel1.GetRow(Jewel2.jewel.JewelPosition, Jewel1.jewel.JewelType, null), Jewel1);
        List<JewelObj> NeiObj2 = Util.ListPlus(Jewel2.GetCollumn(Jewel1.jewel.JewelPosition, Jewel2.jewel.JewelType, null),
        ...
        {
            Util.MoveTo(obj1, obj2.transform.localPosition, 0.2f);
            Util.MoveTo(obj2, obj1.transform.localPosition, 0.2f);
    ...
    {
        if (x - 1 >= 0 && GribManager.cell.GribCellObject[x - 1, y] != null)
            GribManager.cell.GribCellObject[x - 1, y].RemoveEffect();

        if (x + 1 < 7 && GribManager.cell.GribCellObject[x + 1, y] != null)
            GribManager.cell.GribCellObject[x + 1, y].RemoveEffect();

        if (y - 1 >= 0 && GribManager.cell.GribCellObject[x, y - 1] != null)
            GribManager.cell.GribCellObject[x, y - 1].RemoveEffect();

        if (y + 1 < 9 && GribManager.cell.GribCellObject[x, y + 1] != null)
            GribManager.cell.GribCellObject[x, y + 1].RemoveEffect();
    ...
    public void PDestroyRow(int _x, int y)
    {
        ...
        List<CellObject> celleffect = new List<CellObject>();
        List<JewelObj> jeweldes = new List<JewelObj>();
        for (int x = 0; x < 7; x++)
        {
            if (_x != x)
            {
                if (GribManager.cell.GribCellObject[x, y] != null && GribManager.cell.GribCellObject[x, y].cell.CellEffect > 0)
                    celleffect.Add(GribManager.cell.GribCellObject[x, y]);
                if (JewelSpawner.spawn.JewelGribScript[x, y] != null && JewelSpawner.spawn.JewelGribScript[x, y].jewel.JewelType != 99 && GribManager.cell.GribCellObject[x, y].cell.CellEffect == 0)
                    jeweldes.Add(JewelSpawner.spawn.JewelGribScript[x, y]);
            }
        }
        foreach (CellObject item in celleffect)
        {
            item.RemoveEffect();
        }
    ...
    public void DestroyRandom()
    {
        ...
        if (PlayerInfo.MODE == 1)
        {
            if (!isStar)
            {
                List<CellObject> listeff = getListCellEffect();

                if (listeff.Count > 0)
                {
                    CellObject tmp = listeff[Random.Range(0, listeff.Count)];
                    tmp.RemoveEffect();
    ...
    private List<CellObject> getListCellEffect()
    {
        List<CellObject> tmp = new List<CellObject>();
        for (int y = 0; y < 9; y++)
        {
            for (int x = 0; x < 7; x++)
            {
                if (GribManager.cell.GribCellObject[x, y] != null && GribManager.cell.GribCellObject[x, y].cell.CellEffect > 0)
                {
                    tmp.Add(GribManager.cell.GribCellObject[x, y]);
                }
            }
        }
        return tmp;
    }
    private List<CellObject> getListNotEmpty()
    {
        List<CellObject> tmp = new List<CellObject>();
        for (int y = 0; y < 9; y++)
        {
            for (int x = 0; x < 7; x++)
            {
                if (GribManager.cell.GribCellObject[x, y] != null && GribManager.cell.GribCellObject[x, y].cell.CellType > 1)
                {
    ...
    private void destroynotempty()
    {
        try
        {
            List<CellObject> listnotempty = getListNotEmpty();
    ...
    public void AddBonusPower()
    {
        ...
        while (true)
        {
            ...
            JewelObj tmp = JewelSpawner.spawn.JewelGribScript[x, y];
            if (tmp != null && tmp.jewel.JewelType != 8 && tmp.jewel.JewelPower == 0 && GribManager.cell.GribCellObject[x, y].cell.CellEffect == 0)
            {
    ...
    public void ShowStar()
    {
        ...
        for (int y = 9 - 1; y >= 0; y--)
        {
            for (int x = 0; x < 7; x++)
            {
                if (GribManager.cell.GribCellObject[x, y] != null)
    ...
    public void WinChecker()
    {
        ...
        for (int y = 0; y < 9; y++)
        {
            if (GribManager.cell.GribCellObject[(int)JewelStar.jewel.JewelPosition.x, y] != null)
            {
                Min = y;
                break;
            }
